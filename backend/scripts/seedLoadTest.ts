...
async function main() {
  const institution = await prisma.institution.findFirst({ where: { isActive: true }, orderBy: { createdAt: "asc" } });
  ...
  const admin = await prisma.user.findFirst({ where: { institutionId: institution.id, role: { in: ["ADMIN", "SUPER_ADMIN"] } } });
  if (!admin) throw new Error("No existe usuario administrador para createdBy");
  const period = await prisma.period.findFirst({ where: { academicYear: { institutionId: institution.id }, status: "ACTIVE" } });
  ...
  for (let index = 1; index <= STUDENTS; index++) {
    ...
    const user = await prisma.user.upsert({
      ...
      create: { email, passwordHash, firstName: "Carga", lastName: `Alumno ${suffix}`, role: "STUDENT", institutionId: institution.id },
    });
  ...
  let assessment = await prisma.assessment.findFirst({ where: { title: TITLE, courseId: assignment.courseId } });
  if (assessment) {
    ...
    assessment = await prisma.assessment.update({
      ...
      data: { status: "ACTIVE", isActive: true, startDate: new Date(Date.now() - 60_000), endDate: new Date(Date.now() + 24 * 60 * 60 * 1000), allowRetake: false, archivedAt: null },
    });
  } else {
    assessment = await prisma.assessment.create({
      data: {
        courseId: assignment.courseId, subjectId: assignment.subjectId, teacherId: assignment.teacherId,
        periodId: period?.id, title: TITLE, description: "Escenario local k6", assessmentType: "PROCESO",
        deliveryMode: "ONLINE", status: "ACTIVE", semester: 1, maxScore: QUESTIONS, timeLimitMin: 60,
        startDate: new Date(Date.now() - 60_000), endDate: new Date(Date.now() + 24 * 60 * 60 * 1000),
        isActive: true, allowRetake: false, publishedAt: new Date(), createdBy: admin.id,
      },
    });
  }
  ...
  for (let index = 1; index <= QUESTIONS; index++) {
    ...
    let question = await prisma.question.findFirst({ where: { subjectId: assignment.subjectId, statement }, include: { options: true } });
    if (!question) {
      question = await prisma.question.create({
        data: {
          subjectId: assignment.subjectId, type: "MULTIPLE_CHOICE", statement, difficulty: 2, points: 1, createdBy: admin.id,
          options: { create: [
  ...
  console.log(JSON.stringify({ assessmentId: assessment.id, students: STUDENTS, questions: QUESTIONS, csvPath }, null, 2));
}
